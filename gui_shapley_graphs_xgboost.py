...
root.title('Shapely graphs + XGBoost auto tuner')
...
label_frame_shapley = ttk.Labelframe(root, text='Shapley graphs', width=300, height=350)
label_frame_shapley.grid(row=0, column=5, sticky='n')
label_frame_hyperparameters = ttk.Labelframe(root, text='Hyperparameters', width=300, height=350)
label_frame_hyperparameters.grid(row=0, column=6, sticky='n')
label_frame_results = ttk.Labelframe(root, text='Results', width=300, height=350)
label_frame_results.grid(row=0, column=7, sticky='n')
...
def feature_selection():
    ...
    checkbox_scaling_var = tk.BooleanVar(value=False)
...
def SMOTE_scaling():
    ...
    create_shapley_label_frame()

def get_checkbox_values(graph_name):
    """
    Retrieve and print the boolean value of a specific Checkbutton.

    This function iterates through a list of Checkbuttons and retrieves the boolean value
    of the Checkbutton associated with the specified graph_name (shap graphs). It then prints the result
    and returns the boolean value.

    Parameters:
        graph_name (str): The name of the Checkbutton to retrieve the value for.

    Returns:
        bool: The boolean value of the specified Checkbutton.
    """
    # Retrieve and print the boolean values of each Checkbutton
    for i, checkbox_info in enumerate(checkboxes):
        name = checkbox_info["name"]
        if name == graph_name:
            value = checkbox_info["var"].get()
            print(f"Checkbox {i + 1} ({name}): {value}")
        else:
            pass
    return value

def create_shapley_label_frame():
    """
    Create a Shapley label frame with dropdowns and checkboxes.

    This function sets up a label frame for Shapley values with dropdowns for
    the number of individual and combined plots, and checkboxes for different
    Shapley graph functions.

    Returns:
        None
    """

    iter_row = iter(range(20)) # add an extra row

    global dropdown_force_plots, dropdown_stacked_force_plots, checkboxes

    amount_force_plots = list(range(0, 20))
    tk.Label(label_frame_shapley, font="none 7 bold", text="Number of individual plots").grid(row=next(iter_row), column=0, sticky='w')
    dropdown_force_plots = Dropdown(label_frame_shapley, amount_force_plots, amount_force_plots[5], row=next(iter_row), column=0)

    amount_stacked_force_plots = [5, 10, 50, 100, 500, 1000, 5000, 10000, 20000]
    tk.Label(label_frame_shapley, font="none 7 bold", text="Number of combined plots").grid(row=next(iter_row), column=0, sticky='w')
    dropdown_stacked_force_plots = Dropdown(label_frame_shapley, amount_stacked_force_plots, amount_stacked_force_plots[5], row=next(iter_row), column=0)

    shapley_graphs_functions = ["Waterfall", "Force", "Mean", "Beeswarm", "Violin", "Layered_violin", "Correlation_matrix_features", "Shapley_Correlation"]

    # Create a list to store Checkbutton information
    checkboxes = []

    # Loop through each checkbox name
    for row, name in enumerate(shapley_graphs_functions):
        # Use a dictionary to store Checkbutton information
        var = tk.BooleanVar(value=False)
        checkbox = tk.Checkbutton(label_frame_shapley, text=name, variable=var)

        # Store the Checkbutton information in the list
        checkboxes.append({"name": name, "var": var, "checkbox": checkbox})

        # Use the grid method to organize the Checkbuttons
        checkbox.grid(row=next(iter_row), column=0, sticky=tk.W)


def run_optimize(trial, X, y, objective_xgboost):
    """
    ...
    model = xgb.XGBClassifier(**xgb_params, objective= objective_xgboost, random_state=42)
...
def initialize_xgb_model(best_params, objective_xgboost):
    """
...
def explain_model_with_shap(model, X, instance_index=0, feature_index='auto'):
    """
    Create SHAP plots based on the selected options.

    Parameters:
        model (object): The machine learning model for SHAP explanations.
        X (array-like or pd.DataFrame): The input data for which SHAP values will be calculated.
        dropdown_force_plots (Dropdown): The Dropdown widget for the number of individual plots.
    ...
    # retrieves the value of the dropdown
    selected_value_dropdown_force_plots = int(dropdown_force_plots.get_selected_value()) if dropdown_force_plots.get_selected_value() != "None" else None
    print(selected_value_dropdown_force_plots)

    if get_checkbox_values("Waterfall"):
        for number in range(selected_value_dropdown_force_plots):
            shap.plots.waterfall(shap_values[number])

    if get_checkbox_values("Force"):
        shap.plots.force(shap_values[0], matplotlib=True)
        plt.show()
    #  Stacked_force could be added but I did not get it to work

    if get_checkbox_values("Mean"):
        shap.plots.bar(shap_values)
        # Add text annotation below the plot
        shap.summary_plot(shap_values)

    if get_checkbox_values("Beeswarm"):
        shap.plots.beeswarm(shap_values)

    if get_checkbox_values("Violin"):
        shap.plots.violin(shap_values)

    if get_checkbox_values("Layered_violin"):
        shap.plots.violin(shap_values, plot_type="layered_violin")

    # Heatmap could be added but I did not get it to work

    if get_checkbox_values("Correlation_matrix_features"):
    ...
    if get_checkbox_values("Shapley_Correlation"):
        #shap correlations
...
def run_models(X_scaled, y_resampled):
    ...
    #continues or binairy targets:
    unique_targets = y_train.nunique()
    print(unique_targets)
    objective_xgboost = 'reg:squarederror' if unique_targets > 2 else 'binary:logistic'
    print(objective_xgboost)

    study = optuna.create_study(direction='maximize')
    study.optimize(lambda trial: run_optimize(trial, X_train, y_train, objective_xgboost), n_trials=10)
    ...
    best_model = initialize_xgb_model(best_params, objective_xgboost)
